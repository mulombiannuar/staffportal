...
class TicketWorkflow extends Model
{
    ...
    public static function submitTicketHoldingComment($id, $hold_message, $hold_hours, $is_current, $ticket_resolved)
    {
        $ticket = TicketWorkflow::find($id);
        DB::table('ticket_workflows')->where('id', $id)
            ->update([
                'is_current' => $is_current,
                'message_by' => Auth::user()->id,
                'date_responded' => Carbon::now(),
